 ...
 */
global $wp_version;	// JPH - needed to add this for version_compare to work
if ( isset( $wp_version ) && version_compare( $wp_version, '3.5' ) >= 0 ) {
	function matthewruddy_image_resize( $url, $width = NULL, $height = NULL, $crop = true, $retina = false ) {
		...
		// Get default size from database if needed
		...
		if ( !file_exists( $dest_file_name ) ) {
			 ...
			 */
                	...
                	$parts = pathinfo($url);
                	$filename = preg_replace( "/[^a-zA-Z0-9_\-.]/", '', $parts['basename']);
                	$media_url = $parts['dirname'] . '/' . $filename;

			$query = $wpdb->prepare( "SELECT * FROM $wpdb->posts WHERE guid='%s'", $media_url );
